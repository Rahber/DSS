...
$where ="and t.id between '".$_POST['start_ticket']."' and '".$_POST['end_ticket']."'";
$msg="Report By Coupon#";
$order="order by t.id";
}
if($_POST['search']==''){
$sql = $mysqli->query("select t.id,t.t, t.date, p.fname, p.cnic, p.cnic, t.embassy, t.express, t.printedby from userrecord p, printrecord t where p.id=t.uid order by id");
$num=mysqli_num_rows($sql);
$msg="Report By All Passenger";
}else{
if($where!=''){
	
	 $mtc = "select t.id, t.t,t.uid, t.date, p.fname, p.lname, p.cnic, p.cnic, t.embassy, t.express, t.printedby from userrecord p, printrecord t where p.id=t.uid $where $order";
